...
function handleDate(value){
  if(typeof value === "string") {
    return moment(decodeURIComponent(value), [ schema.dateFormatString, moment.ISO_8601 ]);
  }
...
module.exports = function paginatedRequest(path, options){
  ...
  if (params.since) params.since = handleDate(params.since).format(schema.dateFormatString)
  if (params.until) params.until = handleDate(params.until).format(schema.dateFormatString)
